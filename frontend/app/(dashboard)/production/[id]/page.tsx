...
export default function ProductionPlanPage() {
  ...
  async function loadSubRecipeReport() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      await loadPlan();
      // Invalidate reports so they reload with new quantities
  ...
  async function addMeal(meal: MealRecipe) {
    if (!plan) return;
    const newItems = [...plan.items.map((i) => ({ meal_id: i.meal_id, quantity: quantities[i.meal_id] ?? 0 })), { meal_id: meal.id, quantity: 0 }];
    try {
  ...
  async function removeMeal(meal_id: string) {
    if (!plan) return;
    const newItems = plan.items.filter((i) => i.meal_id !== meal_id).map((i) => ({ meal_id: i.meal_id, quantity: quantities[i.meal_id] ?? 0 }));
    try {
      await api.updateProductionPlan(id, { items: newItems });
      await loadPlan();
      setSubRecipeReport(null);
  ...
  const addableMeals = allMeals.filter(
    (m) => !plan.items.find((i) => i.meal_id === m.id) &&
      (m.display_name.toLowerCase().includes(search.toLowerCase()) || m.name.toLowerCase().includes(search.toLowerCase())),
  );
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <button onClick={() => router.push('/production')} className="text-gray-400 hover:text-gray-600 text-sm mb-1 block">← All Plans</button>
          <h1 className="text-2xl font-bold text-gray-900">{plan.week_label}</h1>
          <div className="flex items-center gap-3 mt-1">
            <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${STATUS_STYLES[plan.status]}`}>{plan.status}</span>
            <span className="text-sm text-gray-500">{plan.items.length} meals · {portionedMeals} portioned · {totalPortions.toLocaleString()} total portions</span>
          </div>
        </div>
      ...
      <div className="flex gap-1 mb-6 border-b border-gray-200">
        {([['plan', 'Plan Items'], ['sub-recipes', 'Sub-Recipe Report'], ['shopping', 'Shopping List']] as const).map(([key, label]) => (
          <button
            key={key}
      ...
      {tab === 'plan' && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {plan.items.length === 0 ? (
                  <tr><td colSpan={5} className="px-4 py-10 text-center text-gray-400">No meals added yet</td></tr>
                ) : (
                  plan.items.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        {item.meal.category ? (
                          <span className="px-2 py-0.5 bg-orange-50 text-orange-700 rounded text-xs">{item.meal.category}</span>
                      ...
                      <td className="px-4 py-3">
                        <button onClick={() => removeMeal(item.meal_id)} className="text-xs text-red-400 hover:text-red-600">Remove</button>
                      </td>
                    </tr>
                  ))
                )}
                ...
                {addableMeals.length === 0 ? (
                  ...
                  addableMeals.slice(0, 15).map((meal) => (
                    ...
                    >
                      ...
                      {(meal as any).category && <span className="text-xs text-gray-400 ml-2">{(meal as any).category}</span>}
                    </button>
                  ))
                )}
              </div>
          ...
          {/* Notes */}
          {(plan.notes !== null || notes) && (
            <div className="bg-amber-50 rounded-xl border border-amber-100 p-5">
              <label className="text-xs font-semibold text-amber-800 uppercase tracking-wide block mb-1">Notes</label>
              <textarea
                value={notes}
      ...
      {tab === 'sub-recipes' && (
        <div>
          {reportLoading ? (
            <div className="text-center py-20 text-gray-400">Calculating...</div>
          ) : !subRecipeReport ? (
            <div className="text-center py-20">
              <button onClick={loadSubRecipeReport} className="px-4 py-2 bg-brand-500 text-white text-sm rounded-lg">Generate Report</button>
            </div>
          ) : (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <p className="text-sm text-gray-500">{subRecipeReport.total_sub_recipes} sub-recipes needed across all portioned meals</p>
                <button
                  onClick={() => { setSubRecipeReport(null); loadSubRecipeReport(); }}
                  className="text-xs text-brand-600 hover:underline"
                >
                  Refresh
                </button>
              </div>
              {Object.entries(subRecipeReport.grouped_by_station).sort(([a], [b]) => a.localeCompare(b)).map(([station, rows]) => (
                <div key={station} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="px-2 py-0.5 bg-blue-50 text-blue-700 rounded text-xs font-semibold">{station}</span>
                      <span className="text-sm text-gray-500">{rows.length} sub-recipes</span>
                    </div>
                    <span className="text-gray-400 text-xs">{expandedStations.has(station) ? '▲' : '▼'}</span>
                  </button>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-gray-50">
                          ...
                          <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Sub-Recipe</th>
                          <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Code</th>
                          <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Prod. Day</th>
                          ...
                          <>
                            <tr key={row.id} className="hover:bg-gray-50">
                              <td className="px-4 py-2.5">
                                <span className={`px-1.5 py-0.5 rounded text-xs font-bold ${PRIORITY_COLOR[row.priority] ?? PRIORITY_COLOR[3]}`}>
                                  P{row.priority}
                                </span>
                              </td>
                              ...
                              <td className="px-4 py-2.5 text-right font-semibold text-gray-900">{row.total_quantity.toLocaleString()}</td>
                              <td className="px-4 py-2.5 text-gray-500">{row.unit}</td>
                            </tr>
                            {row.meal_breakdown.length > 1 && (
                              <tr key={`${row.id}-breakdown`} className="bg-gray-50/50">
                                <td colSpan={6} className="px-8 py-1.5">
                                  <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-xs text-gray-400">
                                    {row.meal_breakdown.map((b, i) => (
                                      <span key={i}>{b.meal}: {b.qty.toLocaleString()} {row.unit}</span>
                                    ))}
                                  </div>
                                </td>
                              </tr>
              ...
              {Object.keys(subRecipeReport.grouped_by_station).length === 0 && (
                <div className="text-center py-20 text-gray-400">No sub-recipes found — make sure meals have quantities &gt; 0</div>
              )}
            </div>
          )}
        </div>
      ...
      {tab === 'shopping' && (
        <div>
          {reportLoading ? (
            ...
            <div className="text-center py-20">
              <button onClick={loadShoppingList} className="px-4 py-2 bg-brand-500 text-white text-sm rounded-lg">Generate Shopping List</button>
            </div>
          ) : (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <p className="text-sm text-gray-500">
                  {shoppingList.total_ingredients} ingredients across {Object.keys(shoppingList.grouped_by_category).length} categories
                </p>
                <button onClick={() => { setShoppingList(null); loadShoppingList(); }} className="text-xs text-brand-600 hover:underline">Refresh</button>
              </div>
              {Object.entries(shoppingList.grouped_by_category).sort(([a], [b]) => a.localeCompare(b)).map(([cat, rows]) => (
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-gray-50/50">
                        ...
                        <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Allergens</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                      {rows.map((row) => (
                        <tr key={row.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-2.5 text-right font-semibold text-gray-900">{row.total_quantity.toLocaleString()}</td>
                          <td className="px-4 py-2.5 text-gray-500">{row.unit}</td>
                          <td className="px-4 py-2.5 text-gray-500 text-xs">{row.supplier_name ?? '—'}</td>
              ...
              {Object.keys(shoppingList.grouped_by_category).length === 0 && (
                <div className="text-center py-20 text-gray-400">No ingredients found — make sure meals have quantities &gt; 0</div>
              )}
            </div>
          )}
        </div>
